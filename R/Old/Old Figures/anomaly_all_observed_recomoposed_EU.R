...
df %>%
  filter(
    location %in% c("United Kingdom", "Germany", "Spain", "Italy", "France")
  ) %>%
  select(
    c(date, new_cases, location)
  ) %>%
  rename(
    ds = 1,
    y = 2
...
observed_recomposed
ggsave("/Users/timothywiemken/Library/CloudStorage/OneDrive-Pfizer/Documents/Research/github/COVID forecast/manuscript/JAMA/Figures/anomaly_all_observed_recomposed_EU.pdf", width=10, height=6)
