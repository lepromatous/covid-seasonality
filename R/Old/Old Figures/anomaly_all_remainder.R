...
leg <-  tibble(
  colour = ifelse(yo[yo$key=="remainder","anomaly"]=="Yes", "red", "black"),
  size = ifelse(yo[yo$key=="remainder","anomaly"]=="Yes", 1.5,1),
  size2 = ifelse(yo[yo$key=="remainder","anomaly"]=="Yes", 3,1),
  shape = ifelse(yo[yo$key=="remainder","anomaly"]=="Yes", 1,19),
  size_disappear = ifelse(yo[yo$key=="remainder","anomaly"]=="Yes", 3,1),
  stroke = ifelse(yo[yo$key=="remainder","anomaly"]=="Yes", 1.5,0.3)
)

# replot ----
ggplot(data = yo[yo$key=="remainder",]) + 
  geom_line(
    aes(x= ds, y=value)
  ) +
  geom_point(
  ...
  guides(colour = guide_legend(override.aes = list(size = c(1,5),
                                                   shape = c(19,1),
                                                   stroke = c(0.8, 2)))) -> remainder
remainder
ggsave("/Users/timothywiemken/Library/CloudStorage/OneDrive-Pfizer/Documents/Research/github/COVID forecast/manuscript/JAMA/Figures/anomaly_all_remainder.pdf", width=10, height=6)
