...
leg <-  tibble(
  ...
  size = ifelse(yo[yo$key=="observed","anomaly"]=="Yes", 1.5,1),
  size2 = ifelse(yo[yo$key=="observed","anomaly"]=="Yes", 3,1),
  shape = ifelse(yo[yo$key=="observed","anomaly"]=="Yes", 1,19),
  size_disappear = ifelse(yo[yo$key=="observed","anomaly"]=="Yes", 3,1),
  stroke = ifelse(yo[yo$key=="observed","anomaly"]=="Yes", 1.5,0.8)
)
...
ggplot(data = yo[yo$key=="observed",]) + 
  ...
  scale_x_date(
    ...
    expand=c(0.01, 0.05)) +
  scale_y_continuous(
    limits = c(-3000, 30000), n.breaks=12
  ) + 
  labs(
    colour = "Anomaly",
    x="",
...
observed
ggsave("/Users/timothywiemken/Library/CloudStorage/OneDrive-Pfizer/Documents/Research/github/COVID forecast/manuscript/JAMA/Figures/anomaly_flu_all_observed.pdf", width=10, height=6)
