...
d <- d[!d %in% c("type", "country.lookup")]

type <- sub(".*\\.", "", d)[1]
type <- substr(type, start=1, stop = nchar(type))
...
# country lookup
country.lookup <- data.frame("country" = c(eu.countries))
country.lookup$number <- c(seq(1:length(eu.countries)))
...
d$date <- rep(seq.Date(as.Date("2020-01-01"), by="day", length.out=365), times=length(eu.countries))
d %>%
...
ggplot() +
  ...
  coord_fixed() + 
  scico::scale_fill_scico(midpoint = 0, palette = "cork", direction = -1) + 
...
ggsave(paste0("~/Desktop/", "flu_case", ".pdf"), height = 12)
