...
leg <-  tibble(
  colour = ifelse(yo[yo$key=="trend","anomaly"]=="Yes", "red", "black"),
  size = ifelse(yo[yo$key=="trend","anomaly"]=="Yes", 1.5,1),
  size2 = ifelse(yo[yo$key=="trend","anomaly"]=="Yes", 3,1),
  shape = ifelse(yo[yo$key=="trend","anomaly"]=="Yes", 1,19),
  size_disappear = ifelse(yo[yo$key=="trend","anomaly"]=="Yes", 3,1),
  stroke = ifelse(yo[yo$key=="trend","anomaly"]=="Yes", 1.5,0.3)
)

# replot ----
ggplot(data = yo[yo$key=="trend",]) + 
  # geom_line(
  #   aes(x= ds, y=value)
  # ) +
  geom_point(
  ...
  geom_rect(data = label_range, fill = "#A66923", color = "#f2f2f2", alpha = 0.5,
            aes(xmin = xmin, xmax = xmax, 
                ymin = ymin, ymax = ymax-10,
                group = year)) +
  geom_text(data = label_range,
            aes(x = xmin+30, y = ymin+10,
  ...
  guides(colour = guide_legend(override.aes = list(size = c(1,5),
                                                   shape = c(19,1),
                                                   stroke = c(0.8, 2)))) -> trend
trend
ggsave("/Users/timothywiemken/Library/CloudStorage/OneDrive-Pfizer/Documents/Research/github/COVID forecast/manuscript/JAMA/Figures/anomaly_all_trend.pdf", width=10, height=6)
