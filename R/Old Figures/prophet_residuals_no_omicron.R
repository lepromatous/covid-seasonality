...
forecast <- predict(m) #https://github.com/facebook/prophet/issues/309 predict with no input evaluates model based on history for this. 
...
yo <- merge(test, df, by.x="ds", by.y="week")
residuals <- yo[,'yhat'] - yo[,'rate']
residuals.lo <- yo[,'yhat_lower'] - yo[,'rate']
residuals.hi <- yo[,'yhat_upper'] - yo[,'rate']
...
ggsave("/Users/timothywiemken/Library/CloudStorage/OneDrive-Pfizer/Documents/Research/github/COVID forecast/manuscript/JAMA/Figures/prophet_no_omicron_residuals.pdf", width=10, height=6)
