...
wildtype <- data_frame(
  variant = 'wildtype',
  ds = seq.Date(from=as.Date(min(df$ds)), to = as.Date("2021-06-27"), by="day")
)
delta <- data_frame(
  variant = 'delta',
  ds = seq.Date(from=as.Date("2021-06-28"), to = as.Date("2021-12-13"), by="day")
)
omicron <- data_frame(
  variant = 'omicron',
...
m <- prophet(daily.seasonality= F, 
             ...
             mcmc.samples=1000) ##
# add us holidays ----
m <- add_country_holidays(m, country_name = 'Canada')
