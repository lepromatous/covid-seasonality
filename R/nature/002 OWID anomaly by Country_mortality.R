...
df %>%
  ...
  anomalize(remainder, method = "gesd", alpha = .9, max_anoms = 0.5) %>%
  time_recompose() %>%
...
ggplot(data = yo) + 
  ...
  geom_rect(data = label_range, fill = "#0070BF", color = "#f2f2f2", alpha = 0.5,
            aes(xmin = xmin, xmax = xmax, 
                ymin = ymin+15, ymax = ymax-15,
                group = year)) +
  geom_text(data = label_range,
            aes(x = xmin+25, y = ymin+10,
  ...
  scale_x_date(
    ...
    expand=c(0.01, 0.0)) +
  scale_y_continuous(
    limits = c(-17, (max(yo$observed) + (50 - max(yo$observed) %% 50))), breaks = c(seq(0, (max(yo$observed) + (50 - max(yo$recomposed_l2) %% 10)), by=10))
  ) + 
  labs(
    colour = "Anomaly",
    x="",
