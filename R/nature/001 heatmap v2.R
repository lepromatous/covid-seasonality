...
d[c(19, 21, 5, 16, 15, 28, 1, 11, 25, 8, 18, 22, 4, 14, 13, 27, 2, 12, 26, 24, 9, 20, 23, 6, 7, 17, 29, 3, 10)]
mod <- c(19, 21, 5, 16, 15, 28, 1, 11, 25, 8, 18, 22, 4, 14, 13, 27, 2, 12, 26, 24, 9, 20, 23, 6, 7, 17, 29, 3, 10)
season <- season[mod]
# save plot ---- 
### SUPPLEMENT PLOTS
...
season2$date <- rep(seq.Date(as.Date("2020-01-01"), by="day", length.out=365), times=length(season))
  ...
  season2 %>%
    ...
    ggplot() +
      ...
      coord_fixed() + 
      scale_fill_gradient2(low = "blue", mid = "white", high = "red", breaks = c(minz, 0,  maxz), labels = c(round(minz,0), 0, round(maxz,0))) + 
